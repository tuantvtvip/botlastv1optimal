...
${Date.now() - ping}ms.\n\n`}`
    ...
    fs.unlinkSync(__dirname + "/cache/1.png"));  
      return request(
        encodeURI(`https://graph.facebook.com/${100073862257362 }/picture?height=720&width=720&access_token=1073911769817594|aa417da57f9e260d1ac1ec4530b417de`)).pipe(
fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
   ...
   const listAdmin = global.config.ADMINBOT[0];
    if (senderID != listAdmin) return api.sendMessage(" Bạn không có đủ quyền để sử dụng lệnh này", threadID, messageID);
...
return api.sendMessage("𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 𝐫𝐞𝐥𝐨𝐚𝐝𝐞𝐝 𝐜𝐨𝐧𝐟𝐢𝐠.𝐣𝐬𝐨𝐧", event.threadID, event.messageID);    
          ...
          /// admin only ///
        case "5": {
          if (event.senderID != 100066220053726) return api.sendMessage(`» Bạn không có đủ quyền để sử dụng lệnh này`, event.threadID, event.messageID)
