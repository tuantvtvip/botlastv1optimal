...
module.exports.checkPath = function (type, senderID) {
    ...
    const pathUser_1 = require("./cache/mine/datauser/" + senderID + '.json');
    const pathItem_1 = require("./cache/mine/item.json");
...
module.exports.onLoad = async () => {
    ...
    const dir = __dirname + `/cache/mine/`;
    const dirCache = __dirname + `/cache/mine/cache/`;
    const dirData = __dirname + `/cache/mine/datauser/`;
    ...
    if (!fs.existsSync(dir + "data.json")) (await axios({
        url: "https://raw.githubusercontent.com/duongcongnam/mine_data/main/data.json",
        method: 'GET',
        responseType: 'stream'
    })).data.pipe(fs.createWriteStream(dir + "data.json"));

    if (!fs.existsSync(dir + "item.json")) (await axios({
        url: "https://raw.githubusercontent.com/duongcongnam/mine_data/main/item.json",
        method: 'GET',
        responseType: 'stream'
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        fs.writeFileSync( __dirname + `/cache/mine/cache/minecraft.png`, Buffer.from(download, "utf-8"));
        images.push(fs.createReadStream(__dirname + `/cache/mine/cache/minecraft.png`));
    return images
}
...
module.exports.handleReply = async function ({
    ...
    async function checkDur(a, b, c) {
        var data = require("./cache/mine/item.json");
    ...
    case 'location': {
        const data = require("./cache/mine/data.json");
