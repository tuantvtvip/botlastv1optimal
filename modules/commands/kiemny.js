...
module.exports.run = async function({ api, event, args, Users, Currencies }) {
    ...
    switch (args[0]) {
        ...
        case "breakup":
            ...
            if (Date.now() - userInfo.data.dating.time.origin < 0) return api.sendMessage(`Còn chưa đủ 7 ngày mà đã muốn chia tay là sao? 🥺\n\n${msgBreakup()}\n\nHãy cứ bình tĩnh suy nghĩ, để mọi chuyện dần lắng xuống rồi giải quyết cùng nhau. Nhé? ^^`, threadID, messageID);
        ...
        case "info":
            ...
            `🤵 𝐓ê𝐧 𝐂ủ𝐚 𝐁ạ𝐧: ${userInfo.name}\n` +
            `👸 𝐓ê𝐧 𝐂ủ𝐚 𝐍𝐠ườ𝐢 𝐘ê𝐮: ${infoMates.name}\n` +
            `⏰  𝐓𝐡ờ𝐢 𝐆𝐢𝐚𝐧 𝐁ắ𝐭 Đầ𝐮: ${userInfo.data.dating.time.fullTime}\n` +
            `💑 Đã 𝐁ê𝐧 𝐍𝐡𝐚𝐮: ${parseInt((date2 - date1) / 86400000)} 𝐍𝐠à𝐲\n` +
            `${userInfo.data.dating.lovepoint ? `️🎖️ Đ𝐢ể𝐦 𝐓𝐡â𝐧 𝐓𝐡𝐢ế𝐭: ${userInfo.data.dating.lovepoint} điểm.` : ''}`
