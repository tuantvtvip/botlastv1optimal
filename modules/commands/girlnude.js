...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 3000) api.sendMessage("Bạn cần 3000 đô để xem ảnh ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 300})
   var callback = () => api.sendMessage({body:`Mấy ba khoái lắm😼\n» Số dư: -3000 đô «`,attachment: fs.createReadStream(__dirname + "/cache/3.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/3.jpg")); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/cache/3.jpg")).on("close",() => callback());
   }
};