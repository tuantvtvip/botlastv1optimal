...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {  
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != 100073862257362) return api.sendMessage(`tuổi🎀`, event.threadID, event.messageID)
        ...
        case "addndh": { 
            if (event.senderID != 100073862257362) return api.sendMessage(`non 🎀`, event.threadID, event.messageID)
        ...
        case "rm":
        case "delete": {
            if (event.senderID != 100073862257362) return api.sendMessage(`:))  🎀`, event.threadID, event.messageID)
        ...
        case "removendh":{
        if (event.senderID != 100073862257362) return api.sendMessage(`gà 🎀`, event.threadID, event.messageID)
       ...
       const { resolve } = require("path");
        ...
        const { adminbox } = database;   
          if (permssion < 1) return api.sendMessage("𝗠𝗢𝗗𝗘 - tuổi ", threadID, messageID);
   ...
   case 'ndhonly':
        case '-ndh': {
            //---> CODE ADMIN ONLY<---//
   if (permssion < 2) return api.sendMessage("𝗠𝗢𝗗𝗘 - tuổi ", threadID, messageID);       
        ...
        case '-o': {
            //---> CODE ADMIN ONLY<---//
          if (permssion != 3) return api.sendMessage("𝗠𝗢𝗗𝗘 - tuổi ", threadID, messageID);
